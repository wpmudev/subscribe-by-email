...
function subscribe_by_email_send_instant_notifications($post) {
	...
	$cancel_url = trailingslashit(get_option('siteurl')) . '?action=cancel-subscription&sid=';
	...
	if ($post->post_excerpt && !empty($post->post_excerpt)) {
		...
		$post_excerpt = preg_replace("/&#?[a-z0-9]{2,8};/i","", $post_excerpt);
	} else {
	...
	if (count($subscription_emails) > 0 && get_post_meta($post->ID, 'sbe_notified', true) != 'yes') {
		...
		foreach ($subscription_emails as $subscription_email){
			if ( $done_count > 0 &&
			     get_post_meta($post->ID, 'sbe_notified', true) != 'yes' &&
			     intval(get_post_meta($post->ID, 'sbe_notified', true)) > $i) {
				break;
			}
